...
| Sol | medium；必要时 high | 根因分析、方案设计、跨模块影响；已有证据但仍未形成结论时提高到 high。范围清楚的聚焦任务可选 low |
| Astra | low | Sol High 仍无法解决，或需要跨代码、应用和工具的高难度长流程判断 |

判断不明确时默认 Sol Medium。首次路由不会仅凭任务关键词直达 Astra。Sol Low 失败先升 Medium，Sol Medium 未完成分析时先升 High，Sol High 仍无法解决才升 Astra。明确方案后的新用户执行请求会回到 Luna；重新分析和高风险任务不会因此降级。代理不会自行追踪任务是否已失败；运行中的升级由 Agent 根据实际证据发起。
